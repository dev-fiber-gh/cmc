...
int main(int argc, char* argv[]) {
    ...
    if (argc < 4) {
        ...
        printf("\tapp c infile outfile\n");
        printf("\tapp d infile outfile\n");
        exit(TooFewArgumentsError);
    }
    
    
    if (!strcmp(argv[2], argv[3])) { printf("Files have same names.\n"); exit(SameNamesError); }
    ...
    if (argv[1][0] == L'c') {
        ...
        if((fopen(argv[3], "rb"))) { 
            printf("Output file does exist! Programm will not rewrite it.\n"); exit(OutputFileError);
        }
